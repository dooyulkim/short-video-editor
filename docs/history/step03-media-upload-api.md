...
- **`extract_video_metadata(file_path)`** - Extracts video metadata using OpenCV and MoviePy
  - Duration, width, height, FPS, codec, format
  - Checks for audio track presence
- **`extract_audio_metadata(file_path)`** - Extracts audio metadata using MoviePy
...
- `moviepy==1.0.3` (compatible with Python 3.8)
- `decorator<5.0,>=4.0.2` (required by moviepy 1.0.3)
