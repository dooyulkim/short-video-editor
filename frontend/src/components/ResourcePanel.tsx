...
import { Upload, Video, Music, Image as ImageIcon, Clock, Maximize2 } from 'lucide-react';
...
export const ResourcePanel: React.FC<ResourcePanelProps> = ({ onResourcesChange }) => {
    ...
    <div className="h-full flex flex-col p-4 bg-background">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Upload media files or drag them to the timeline
        </p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            or click to browse (Video, Audio, Images)
          </p>
        </div>
      </div>
      ...
      {/* Filter Tabs */}
      <Tabs value={filter} onValueChange={(value: string) => setFilter(value as FilterType)} className="flex-1 flex flex-col">
                  ...
                  <CardContent className="p-3 pt-0">
                    ...
                    {hoveredResource === resource.id && (
                      <CardDescription className="space-y-1 text-xs animate-in fade-in duration-200">
                        ...
                        {resource.metadata.fps && (
                          <div>FPS: {resource.metadata.fps}</div>
                        )}
                        {resource.metadata.codec && (
                          <div className="truncate">Codec: {resource.metadata.codec}</div>
                        )}
                      </CardDescription>
                    )}
                  </CardContent>
