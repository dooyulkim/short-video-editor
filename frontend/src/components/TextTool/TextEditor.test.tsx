...
describe("TextEditor Component", () => {
	...
	describe("Rendering", () => {
		...
		it("should render custom trigger when provided", () => {
			render(<TextEditor onAddText={mockOnAddText} trigger={<button>Custom Button</button>} />);
	...
	describe("Form Interactions", () => {
		...
		it("should update duration when changed", async () => {
			...
			await userEvent.tripleClick(duration); // Select all text
			await userEvent.keyboard("10"); // Type replacement
	...
	describe("Form Submission", () => {
		...
		it("should call onAddText with correct data when submitted", async () => {
			...
			const positionX = screen.getByLabelText(/position x/i);
			await userEvent.tripleClick(positionX);
			await userEvent.keyboard("150");

			const positionY = screen.getByLabelText(/position y/i);
			await userEvent.tripleClick(positionY);
			await userEvent.keyboard("250");

			const duration = screen.getByLabelText(/duration/i) as HTMLInputElement;
			await userEvent.tripleClick(duration);
