...
export function TextEditor({ onAddText, trigger }: TextEditorProps) {
  ...
  const updateTextProp = <K extends keyof TextProperties>(
    key: K,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger || <Button variant="outline">Add Text</Button>}
      </DialogTrigger>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add Text to Timeline</DialogTitle>
          <DialogDescription>
            Configure text properties and add it to your video timeline.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-6 py-4">
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                updateTextProp('duration', parseFloat(e.target.value) || 1)
              }
              min={0.1}
              step={0.1}
            />
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(value: 'none' | 'fade' | 'slide') =>
                updateTextProp('animation', value)
              }
            >
          ...
          <div className="grid gap-2">
            <Label>Preview</Label>
            <div className="border rounded-lg p-8 bg-black/90 min-h-37.5 flex items-center justify-start relative overflow-hidden">
              <div
                style={{
                  fontFamily: textProps.fontFamily,
                  fontSize: `${textProps.fontSize * 0.5}px`, // Scale down for preview
                  color: textProps.color,
                  position: 'absolute',
                  left: `${textProps.x * 0.3}px`, // Scale position for preview
                  top: `${textProps.y * 0.3}px`,
                  whiteSpace: 'pre-wrap',
                  wordBreak: 'break-word',
                }}
              >
                {textProps.text || 'Enter text here'}
              </div>
            </div>
            <p className="text-sm text-muted-foreground">
              Preview is scaled down. Actual text will appear larger on the video.
            </p>
          </div>
        </div>
