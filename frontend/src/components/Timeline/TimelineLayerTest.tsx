  ...
  const handleClipTrim = (clipId: string, newDuration: number, newTrimStart: number) => {
    setLayers((prevLayers) =>
      prevLayers.map((layer) => ({
        ...
        clips: layer.clips.map((clip) =>
          clip.id === clipId
            ? { ...clip, duration: newDuration, trimStart: newTrimStart }
            : clip
        ),
      }))
    );
    ...
    <div className="w-full h-screen bg-slate-900 text-white p-4">
      ...
      <Card className="bg-slate-800 border-slate-700 overflow-hidden">
        <div className="flex border-b border-slate-600">
          <div className="w-48 bg-slate-700 p-2 text-center font-semibold border-r border-slate-600">
            Layers
          </div>
          <div className="flex-1 bg-slate-700 p-2 text-center font-semibold">
            Timeline (0s - 20s)
          </div>
        </div>
