...
describe('useAudioWaveform', () => {
  ...
  it('should not update state after unmount', async () => {
    ...
    const { result, unmount } = renderHook(() => useAudioWaveform(audioId));
  ...
  it('should maintain separate cache entries for different audioIds', async () => {
    ...
    // Fetch second audio
    const { result: result2 } = renderHook(() => useAudioWaveform(audioId2));
    await waitFor(() => expect(result2.current.isLoading).toBe(false));
    
    // Fetch first audio again - should use cache
    const { result: result3 } = renderHook(() => useAudioWaveform(audioId1));
