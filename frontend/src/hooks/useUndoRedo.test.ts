...
describe("useUndoRedo", () => {
	...
	it("should initialize with no undo/redo available", () => {
		...
		const { result } = renderHook(() => useUndoRedo(initialState, mockRestoreState));
	...
	it("should add states to history", () => {
		...
		expect(result.current.canUndo).toBe(true); // Position 1, can undo to position 0
		expect(result.current.canRedo).toBe(false);
		...
		expect(result.current.canUndo).toBe(true); // Can still undo to previous state
		expect(result.current.canRedo).toBe(false);
	});
	...
	it("should undo to previous state", () => {
		...
		const { result, rerender } = renderHook(({ currentState }) => useUndoRedo(currentState, mockRestoreState), {
		...
		expect(result.current.canUndo).toBe(true); // At position 1, can undo to position 0
	...
	it("should handle undo when at beginning of history", () => {
		...
		// We're now at position 1, can undo to position 0
		act(() => {
			result.current.undo();
		});

		expect(mockRestoreState).toHaveBeenCalled(); // Should restore to position 0
		expect(result.current.canUndo).toBe(false); // Now at position 0, cannot undo further
	});
	...
	it("should create deep copies of states to prevent mutations", () => {
		...
		const { result } = renderHook(() => useUndoRedo(state1, mockRestoreState));
